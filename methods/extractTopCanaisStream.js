...
module.exports = async (args, callback) => {
  const extractTopCanaisStream = async ({ page, data }) => {
    let requestURI = null;
    const url = data.url;
    ...
    await Promise.all([
      page.waitFor(10000),
      page.click(
        'div.wp-block-button.aligncenter.is-style-fill.canais > a:first-child'
      ),
    ]);
  ...
  try {
    ...
    const requestURI = await args.puppeteer.execute(
      args,
      extractTopCanaisStream
    );
    logger.info(
      'Pattern "%s" %s in %s',
      args.regex,
